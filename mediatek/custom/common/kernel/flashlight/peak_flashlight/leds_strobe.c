...
static void flashlight_timer_func(unsigned long param)
{
	...
	printk("XXXXXX GPIO_CAMERA_FLASH_EN,GPIO_OUT_ZERO XXXXXX\n");
}
    ...
    ssize_t gpio_FL_Disable(void) {
		if (g_strobe_On) {
			mod_timer(&flash_timer, jiffies + 120);
			printk("XXXXXX gpio_FL_Disable XXXXXX\n");
		}
        return 0;
    }
...
static int peak_flashlight_open(void *pArg)
{
	...
	flash_timer.function = flashlight_timer_func;
